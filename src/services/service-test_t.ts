export default class GetTestData {
  ...
  waitTimeoutErr = async (ms: number) => {
    setTimeout(() => {
  ...
  idGenerator = ():string => {
    ...
    const ax: number = Math.floor(Math.random() * 7);
    const bx: number = Math.floor(Math.random() * 7);
    const cx: number = Math.floor(Math.random() * 7);
    const dx: number = Math.floor(Math.random() * 7);
    const ex: number = Math.floor(Math.random() * 7);
    const fx: number = Math.floor(Math.random() * 7);

    if (ax + bx < 10) {
      const abx: number = ax * bx + (bx + cx * (217 - bx)) * (ax + 7) + 15 * ax * bx;
      const bbx: number = bx + cx + ax * (300 + ax + dx + bx - fx);
      const mix: number = abx * bbx - dx * ax * 5;
