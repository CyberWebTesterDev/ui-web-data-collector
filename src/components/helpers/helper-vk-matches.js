...
import { showPopup } from "./popup-util";
  ...
  if (profileId && val) {
    ...
    if (key === "relationship") {
      ...
      if (res.rowCount) {
        ...
        console.warn(`Check the relationship property in DB for ${profileId}`);
      }
    }
  } else {
...
export const SimpleCheckerChild = ({ id }) => {
  return (
    <React.Fragment>
      <select
        id={id}
        onChange={(e) => serviceHandler(e.target.id, e.target.value, "child")}
      >
